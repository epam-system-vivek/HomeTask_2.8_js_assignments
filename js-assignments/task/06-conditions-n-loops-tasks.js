...
function isBracketsBalanced(str) {
    const brackets = {
        ...
        if (brackets[lastOpeningBracket] !== currentChar) {
        return false;
        }
        }
        }
...
function toNaryString(num, n) {
  if (num === 0) {
    ...
    const digit = num % n;
    const rest = Math.floor(num / n);
    return toNaryString(rest, n) + digit;
  }
}
...
function getCommonDirectoryPath(pathes) {
    const pathLevels = pathes.map((path) => path.split("/"));

  const shortestPathLevels = pathLevels.reduce((acc, curr) =>
    acc.length > curr.length ? curr : acc
  );

  let commonPath = "";

  for (let i = 0; i < shortestPathLevels.length; i++) {
    const level = shortestPathLevels[i];

    for (let j = 0; j < pathLevels.length; j++) {
      const path = pathLevels[j];
      if (path[i] !== level) {
        return commonPath;
      }
    }
